...
#include "insn_trans/trans_rvwch.c.inc"
...
static void decode_opc(CPURISCVState *env, DisasContext *ctx)
{
    ...
    if (ctx->cur_insn_len == 2) {
         ...
         */
        /*
         * XW 复用 RV32C 的 c.fld/c.fsd、c.fldsp/c.fsdsp 等槽位；无 D 时标准译码
         * 无法消费 001…10 / 101…10，须先试 WCH 译码（与 binutils -M xw 一致）。
         */
        if ((has_ext(ctx, RVC) || ctx->cfg_ptr->ext_zca) &&
            ((ctx->cfg_ptr->ext_xw &&
              (decode_wch_xw16(ctx, (uint16_t)opcode) ||
               decode_wch_xw16_quad2(ctx, (uint16_t)opcode))) ||
        ...
        /* WCH QingKe mcpy (32-bit, opcode=0x0F/func3=7) 先于标准 decode_insn32 */
        if (ctx->cfg_ptr->ext_xwchmcpy && decode_wch_mcpy32(ctx, opcode)) {
            return;
        }
