                                  ...
                                  uintptr_t ra)
{
    ...
    /*
     * 标准 RISC-V 规范：PMP 使能但无规则时，S/U 模式访问全部拒绝。
     * WCH QingKe 差异：PMP 规则为空时 S/U 模式访问全部允许（PMP 只在
     * 用户主动配置保护后才生效，未配置时不应阻止 mret 到 S 模式）。
     * ext_hpe 标志标识所有 WCH QingKe V2/V3/V4/V5 CPU。
     */
    if (riscv_cpu_cfg(env)->pmp &&
        !pmp_get_num_rules(env) && (prev_priv != PRV_M) &&
        !riscv_cpu_cfg(env)->ext_hpe) {
...
{
    /*
     * 先保存 SW_Handler 标志（mret_pop 会清除它），用于后续判断是否退出 VTF 计数。
     * SW_Handler（MSIP/portYIELD）入口时未递增该计数器，
     * mret 时也不应递减，否则会错误地将外层 VTF ISR 的计数减掉。
     */
    bool was_sw_handler = env->wch_hpe_sw_handler_skip;
    wch_qingke_hpe_mret_pop(env, riscv_cpu_cfg(env));

    /*
     * WCH PFIC 同优先级中断不嵌套：仅 VTF 异步中断（入口时递增了计数器）
     * mret 时才递减。SW_Handler 入口时未递增，mret 时跳过递减。
     * 详见 wch-qingke.h wch_vtf_exit()。
     */
    if (wch_is_vtf_mode(env, riscv_cpu_cfg(env)) && !was_sw_handler) {
        wch_vtf_exit(env);
        /*
         * VTF 层 mret 后：若有 PFIC mret 回调，通知 machine 清理
         * wch_evt_mcause_override + MEIP，并重新扫描套装中断。
         * 此回调在 BQL 持有期间执行（TCG 环境）。
         */
        if (env->wch_pfic_mret_cb && env->wch_evt_mcause_override) {
            env->wch_pfic_mret_cb(env, env->wch_evt_mcause_override,
                                  env->wch_pfic_opaque);
        }
    }
