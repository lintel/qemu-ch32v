...
static RISCVException write_mtvec(CPURISCVState *env, int csrno,
                                  target_ulong val, uintptr_t ra)
{
    const RISCVCPU *cpu = env_archcpu(env);

    /* bits [1:0] encode mode; 0 = direct, 1 = vectored, 2 >= reserved */
    if ((val & 3) < 2) {
        env->mtvec = val;
    } else if ((val & 3) == 3 && cpu->cfg.ext_hpe) {
        /* WCH CH32: mtvec[1:0]==3 (VTF + hardware prologue). */
        env->mtvec = val;
    } else {
...
/* ===========================================================
 * WCH QingKe 定制 CSR 实现
 * 0x800 = gintenr  ： mstatus 镜像，读回不含 SD bit31
 * 0x804 = intsyscr ： HWSTKEN|INESTEN|PMTCFG|GIHWSTKNEN 读写存储
 * =========================================================== */

static RISCVException read_wch_gintenr(CPURISCVState *env, int csrno,
                                        target_ulong *val)
{
    /* 返回 mstatus 原始存储局，不调用 add_status_sd，避免 bit31 SD 被置位 */
    *val = (target_ulong)env->mstatus;
    return RISCV_EXCP_NONE;
}

static RISCVException read_wch_intsyscr(CPURISCVState *env, int csrno,
                                         target_ulong *val)
{
    *val = env->wch_intsyscr;
    return RISCV_EXCP_NONE;
}

static RISCVException write_wch_intsyscr(CPURISCVState *env, int csrno,
                                          target_ulong val, uintptr_t ra)
{
    /* 只保留已定义位：HWSTKEN(0)|INESTEN(1)|PMTCFG[3:2]|GIHWSTKNEN(4) */
    env->wch_intsyscr = (uint32_t)(val & 0x0000001Fu);
    return RISCV_EXCP_NONE;
}
...
riscv_csr_operations csr_ops[CSR_TABLE_SIZE] = {
    ...
    [CSR_SCOUNTOVF]      = { "scountovf", sscofpmf,  read_scountovf,
                             .min_priv_ver = PRIV_VERSION_1_12_0 },
    /* WCH QingKe: startup_ch32v30x_D8C.S touches these custom CSRs */
    /*
     * CSR 0x800 = gintenr: 全局中断使能寄存器。
     * 是 mstatus 中 MIE(bit3)/MPIE(bit7) 的用户模式可访问镜像。
     * 真机读回值如 0x7888：bit3=MIE=1, bit7=MPIE=1 等。
     * 固件通过 csrs/csrc 0x800, 0x88 来使能/禁用全局中断（__enable_irq/__disable_irq）。
     *
     * 实现：读返回 mstatus 的 MIE/MPIE 位；写时设置/清除 mstatus 对应位。
     * csrs 0x800, t0 -> 读取旧值，然后 OR t0 到 gintenr（设置 MIE）
     * csrc 0x800, t0 -> 读取旧值，然后 AND ~t0 到 gintenr（清除 MIE）
     */
    [0x800] = { "wch_gintenr", any, read_wch_gintenr, write_mstatus },
    /*
     * CSR 0x804 = INTSYSCR: 中断系统控制寄存器。
     * bit0: HWSTKEN  - 硬件压栈使能
     * bit1: INESTEN  - 中断嵌套使能
     * bit[3:2]: PMTCFG - 嵌套深度配置（V4F only）
     * bit4: HWSTKOVEN - 硬件压栈溢出后中断使能
     * bit5: GIHWSTKNEN - 全局中断和硬件压栈关闭使能（RTOS 用）
     * bit[15:8]: PMTSTA - 抢占位状态指示（只读）
     *
     * 真机值 0x13 = HWSTKEN(1) | INESTEN(1) | GIHWSTKNEN(0x10)。
     * 在 QEMU 中存储为 env->wch_intsyscr，以便启动代码读写不触发异常。
     */
    [0x804] = { "wch_intsyscr", any, read_wch_intsyscr, write_wch_intsyscr },
    [0xbc0] = { "wch_misc", any, read_zero, write_ignore },
