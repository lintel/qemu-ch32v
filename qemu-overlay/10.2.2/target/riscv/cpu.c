...
static void riscv_cpu_reset_hold(Object *obj, ResetType type)
{
    ...
    /*
     * WCH QingKe HPE / VTF 状态复位：
     * WDT 等 guest reset 后，HPE shadow bank 深度、VTF ISR 计数器和
     * SW_Handler 跳过标志必须清零，否则复位前中断被打断的残留状态
     * 会导致 FreeRTOS portYIELD 被永久推迟（wch_vtf_in_isr 残留 > 0）
     * 或 HPE pop/push 深度不对称，使新一轮启动直接死机。
     * mtvec 同样清零，强制固件 startup 重新写入，避免旧值在 startup
     * 初始化 mtvec 之前被意外使用。
     */
    env->mtvec = 0;
    env->wch_hpe_depth = 0;
    env->wch_vtf_in_isr = 0;
    env->wch_hpe_sw_handler_skip = false;
    memset(env->wch_hpe_bank, 0, sizeof(env->wch_hpe_bank));
    env->wch_intsyscr = 0;  /* 固件启动代码会写入 0x13，guest reset 时清零 */
...
static void riscv_cpu_set_irq(void *opaque, int irq, int level)
{
    ...
    if (irq < IRQ_LOCAL_MAX) {
        ...
        } else if (cpu->cfg.ext_hpe && riscv_cpu_is_32bit(cpu)) {
            /*
             * WCH CH32：gpio 64..94 对应 PFIC 扩展 NVIC 线；用 MEIP 递送并在
             * wch_evt_mcause_override 记下 EVT 字索引（见 cpu_helper VTF）。
             */
            if (level) {
                env->wch_evt_mcause_override = (uint32_t)irq;
...
const RISCVCPUMultiExtConfig riscv_cpu_extensions[] = {
    ...
    MULTI_EXT_CFG_BOOL("xw", ext_xw, false),
    MULTI_EXT_CFG_BOOL("xwchmcpy", ext_xwchmcpy, false),
    MULTI_EXT_CFG_BOOL("hpe", ext_hpe, false),
...
static const TypeInfo riscv_cpu_type_infos[] = {
    ...
    /*
     * BASE32（"rv32"）：标准 32 位通用基类，继承 DYNAMIC_CPU（有 MMU/SV32 支持）。
     * 用于 spike / virt / microblaze-v 等标准机器的默认 CPU 类型。
     * WCH QingKe 系列不再继承此类，改用 TYPE_RISCV_CPU_WCH_QINGKE_BASE。
     */
    ...
    /*
     * WCH QingKe 系列 CPU 定义（抽象基类 + 13 个 V2/V3/V4/V5 型号 + 3 个公共配置宏）。
     *
     * 为降低 QEMU 版本升级（cpu.c 上游演进）带来的 overlay 合并冲突面，
     * 将 WCH 私有的 ~210 行 CPU 定义抽离到独立文件 wch-qingke-cpu.c.inc，
     * 通过 #include 嵌入到 riscv_cpu_type_infos[] 数组初始化器中。
     *
     * 该 .inc 文件仅是文本片段（非独立编译单元），不需要加入 meson.build。
     * 新增/调整 QingKe 型号请直接修改该 .inc，本文件无需改动。
     */
#include "wch-qingke-cpu.c.inc"
