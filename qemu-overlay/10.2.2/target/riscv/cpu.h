...
struct CPUArchState {
    ...
    /*
     * WCH QingKe V4 HPE: shadow caller-saved GPR + ra (see GCC
     * riscv_wch_fast_interrupt_saved_reg). Up to 8 nested async M-mode
     * interrupts when ext_hpe is enabled.
     */
    uint8_t wch_hpe_depth;
    target_ulong wch_hpe_bank[8][16];
    /*
     * WCH CH32 SW_Handler（MSIP 路径， vec_word=14）是普通汇编 ISR，自己
     * 管理所有寄存器（用于 FreeRTOS 任务切换）。进入时不需要 HPE push，
     * mret 时也不需要 HPE pop（否则 pop 会覆盖已恢复的新任务寄存器）。
     * 当 cause==IRQ_M_SOFT 时 cpu_helper 设置此标志，post-mret 时自动清除。
     */
    bool wch_hpe_sw_handler_skip;
    /*
     * WCH PFIC 同优先级中断不嵌套语义：VTF 异步中断（如 SysTick_Handler）
     * 入口时递增，mret 时递减。MSIP（SW_Handler）处理时若此值 > 0 则推迟，
     * 等当前 VTF ISR mret 后（计数降为 0）再响应。
     * 这模拟真实 WCH PFIC 硬件中同优先级中断不嵌套的行为，防止 MSIP 在
     * SysTick_Handler 开中断后、切回任务栈之前被嵌套处理导致 sp 损坏。
     */
    uint32_t wch_vtf_in_isr;
    /*
     * WCH CH32：板级用 MIP_MEIP（RISC-V irq 11）递送 PFIC 高编号中断时，
     * VTF（mtvec mode 3）下 EVT 字索引不能为 11，须用此处保存的 NVIC 线号
     *（如 USBHS_IRQn=85）。为零表示 MEIP 非本机制置位。
     */
    uint32_t wch_evt_mcause_override;
    /*
     * WCH PFIC mret 回调：mret 时为非零 override。
     * 由 machine 在 init 时注册，在当前 VTF 层 mret 后清理 MEIP + override。
     * 参数：env（CPU 环境）、irq_n（将要清除的 NVIC 编号）、opaque（机器指针）
     */
    void (*wch_pfic_mret_cb)(CPURISCVState *env, uint32_t irq_n, void *opaque);
    void *wch_pfic_opaque;

    /*
     * WCH QingKe V4F 中断系统控制寄存器（CSR 0x804 = INTSYSCR）。
     * bit0: HWSTKEN  - 硬件压栈使能（HPE）
     * bit1: INESTEN  - 中断嵌套使能
     * bit[3:2]: PMTCFG - 嵌套深度配置（V4F only）
     * bit4: GIHWSTKNEN - 全局中断和 HPE 封锁使能（RTOS 用）
     * 真机典型值 0x13 = HWSTKEN|INESTEN|GIHWSTKNEN。
     */
    uint32_t wch_intsyscr;
